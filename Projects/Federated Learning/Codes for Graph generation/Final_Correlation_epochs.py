...
def Local_fn(x,y,global_model,round_no,worker_no):
    ...
    class LSTM_net(nn.Module):
        def __init__(self):
            ...
            self.lstm1=nn.LSTM(input_size=27,hidden_size=30,bias=False)
...
pred_size=4         #fixed forecast size everytime
...
for n in range (0,No_comm_rounds):
    if n ==0 :
        ...
        for i in range(0,len(Dataset)):          
            ...
            model,avg_loss = Local_fn(x_numpy,y_numpy,0,n,i)
        ...
        for i in range(0,len(Dataset)):          
            ...
            x_numpy=np.asarray(xs1,dtype="float64")
            y_numpy=np.asarray(ys1,dtype="float64")
            model,avg_loss = Local_fn(x_numpy,y_numpy,global_model,n,i)
            temp_model= model.state_dict()
            w_locals.append(copy.deepcopy(temp_model)) 
            Losses.append(avg_loss)
        global_model = FedAvg(w_locals,N)       
...
plt.plot(global_loss , color = 'r', label ='E = 20 , B = 10 ')     
np.savetxt("global_loss_withoutcorrelation_Epoch_20_Batch_10.csv", global_loss)  
torch.save(global_model,"GLOBALMODELNEWONE_withoutCorrelationEpoch_20_Batch_10.pt" ) 
...
plt.plot(global_loss , color = 'b', label ='E = 10 , B = 20 ')     
np.savetxt("global_loss_withoutcorrelation_Epoch_10_Batch_20.csv", global_loss)  
torch.save(global_model,"GLOBALMODELNEWONE_withoutCorrelationEpoch_10_Batch_20.pt" ) 
...
plt.plot(global_loss , color = 'g', label ='E = 5 , B = 10 ')     
np.savetxt("global_loss_withoutcorrelation_Epoch_5_Batch_10.csv", global_loss)  
torch.save(global_model,"GLOBALMODELNEWONE_withoutCorrelationEpoch_5_Batch_10.pt" ) 
...
plt.plot(global_loss , color = 'y', label ='E = 1 , B = 10 ')     
np.savetxt("global_loss_withoutcorrelation_Epoch_1_Batch_10.csv", global_loss)  
torch.save(global_model,"GLOBALMODELNEWONE_withoutCorrelationEpoch_1_Batch_10.pt" ) 
...
plt.savefig('FedAvgCorr_Loss_vs_Communicationrounds_withoutcorrelation_Batches_Epochs_NEWONE.png') 
